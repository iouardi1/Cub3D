...
/*   Updated: 2023/01/08 00:31:52 by iouardi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int touch_wall(t_data *game_data, int pixel_x, int pixel_y)
{
	...
	if (pixel_x < 0 || pixel_y < 0 || block_x > (int)game_data->map.length - 1 || block_y > (int)game_data->map.height - 1)
		return 1;
...
t_coordinates last_h_inter(t_data *game_data, t_coordinates last_h_inter, t_angles angles)
{
	double x_dists;
	double y_dist;
	...
	while (1)
	{
		if ((angles.sin > 0 && touch_wall(game_data, last_h_inter.x, last_h_inter.y - 1)) || (angles.sin <= 0 && touch_wall(game_data, last_h_inter.x, last_h_inter.y)))
			break;
...
t_coordinates last_v_inter(t_data *game_data, t_coordinates last_v_inter, t_angles angles)
{
	double x_dists;
	double y_dist;
	...
	while (1)
	{
		if ((angles.cos < 0 && touch_wall(game_data, last_v_inter.x - 1, last_v_inter.y)) || (angles.cos >= 0 && touch_wall(game_data, last_v_inter.x, last_v_inter.y)))
			break;
