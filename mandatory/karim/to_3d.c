...
/*   Updated: 2023/01/08 00:33:18 by iouardi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ray_casting(t_data *game_data)
{
	...
	while (x >= 0)
	{
		inter = ray_cast(game_data, deg);
		inter.correct_dist = get_distance(game_data, to_coords(inter.x, inter.y)) * cos(game_data->player.angle - deg);
...
void textures_init(t_data *game_data)
{
	...
	while (i++ < 4)
	{
		...
		game_data->mlx_settings.imgs[i].addr = mlx_get_data_addr(game_data->mlx_settings.imgs[i].img,
